...
class mousebase:
    def __init__(self):
        self.button_name = ''
        self.modifiers = bitset()
	    self.eventtype = bitset()
	    self.buttons = bitset()
		...
		self.modifiers_t = enum(shift, ctrl, alt, command)
	    #'eventtype' contains state flags as defined by 'event'.
	    self.event_t = enum( press, release, click, drag, drop )
        self.button_t = enum( left, right, middle )
	...
	def get_buttons(self) :
        if (self.buttons.test( self.left))
    		return ( "left")
    	else if (self.buttons.test( self.right))
    		return ( "right")
    	else if (self.buttons.test( self.middle))
    		return ( "middle")
    	else
    		return 0
    @property
    ...
    @rightdown.setter
	def set_rightdown(self, _rd) :
        self.buttons.set( self.right, _rd)

    @middledown.setter
	def set_middledown(self, _md) :
...
class mousebutton:
    def __init__(self, down = False, dragging = False, last_down_x = -1.0f, \
     last_down_y = -1.0f):
    	self.down = down
