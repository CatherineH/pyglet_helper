...
class mouse_manager:
    def __init__(self, display = display, px = 0, py = 0, left_down = False, \
     ...
     right_dragging = False, right_semidrag = False):
        ...
        self.mouse = mouse()
	def report_mouse_state(self, physical_button_count, is_button_down, old_x,\
       old_y, new_x, new_y, shift_state_count, shift_state ):
        '''
    ...
    def update(self, new_buttons, old_px, old_py, new_px, new_py, new_shift):
	    ...
	    self.mouse.set_shift( new_shift[0] )
	    self.mouse.set_ctrl( new_shift[1] )
	    self.mouse.set_alt( new_shift[2] )
	    self.mouse.set_command( new_shift[3] )

	    if (new_buttons[1]): # handle spin or zoom if allowed
            if new_buttons[0]:
                self.display.report_camera_motion( (new_px - old_px), (new_py - old_py), display_kernel::MIDDLE)
            else:
                self.display.report_camera_motion( (new_px - old_px), (new_py - old_py), display_kernel::RIGHT )
    	...
    	if not new_buttons[1]) : #< Ignore changes in the left button state while the right button is down!
    		...
    		self.left_down = b
    	if (not self.display.spin_is_allowed() and not new_buttons[0]) : #< Ignore changes in the left button state while the right button is down!
    		b = new_buttons[1]
