...
class apply_material:
    def __init__(self, v, m, material_matrix, sp = None) shader_program()):
        self.v = v
        self.m = m
	    ...
	    if ( (loc = self.m.shader.get_uniform_location( self.v, "model_material" )) >= 0 ):
            self.m.shader.set_uniform_matrix( self.v, self.loc, self.model_material )
	    if ( (loc = self.m.shader.get_uniform_location( self.v, "light_count" )) >= 0 ):
            self.v.glext.glUniform1iARB( self.loc, self.v.light_count[0] )
        if ( (loc = self.m.shader.get_uniform_location( self.v, "light_pos" )) >= 0 and self.v.light_count[0]):
            self.v.light_pos[0] = self.v.glext.glUniform4fvARB( loc, self.v.light_count[0])
        if ( (loc = self.m.shader.get_uniform_location( self.v, "light_color" )) >= 0 and self.v.light_count[0] ):
